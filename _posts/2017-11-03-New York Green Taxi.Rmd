...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# y$hour_dropoff = as.numeric(y$hour_dropoff)
...
#     column(6,
#            fluidRow(
#              column(6,h5("Visualization of New York Green Cab dropoff on September 2, 2015."))
#            ),
#            fluidRow(
#              column(6,sliderInput("range_passenger",label = "Passengers",
#                                   value = range(y$Passenger_count), 
#                                   min = min(y$Passenger_count), 
#                                   max = max(y$Passenger_count), 
#                                   step = 1))
#            ),
#            fluidRow(
#              column(6,sliderInput("range_fare",label = "Fare Amount",
#                                   value = range(y$Total_amount), 
#                                   min = min(y$Total_amount), 
#                                   max = max(y$Total_amount), 
#                                   pre = '$'))
#            ),
#            fluidRow(
#              column(6,sliderInput("range_distance",label = "Distance",
#                                   value = range(y$Trip_distance), 
#                                   min = min(y$Trip_distance), 
#                                   max = max(y$Trip_distance), 
#                                   post = 'miles'))
#            ),
#            fluidRow(
#              column(6,sliderInput("range_hour",label = "Hours",
#                                   value = min(y$hour_dropoff), 
#                                   min = min(y$hour_dropoff), 
#                                   max = max(y$hour_dropoff),
#                                   animate = TRUE,
#                                   step = 1))
#            ),
#            fluidRow(
#              column(3,checkboxGroupInput("payment_type",label = "Payment Type",
...
#                                          selected = c(1,2))),
#              column(3,checkboxGroupInput("rate_code",label = "RateCodeID",
#                                          choices = list("Standard"=1,"JFK"=2, "Newark"=3, "Nassau/Westchester"=4, "Negotiate"=5, "Group"=6),
#                                          selected = c(1)))
#            )
#     )
#   )
...
#     subset(y,Passenger_count>=input$range_passenger[1] & Passenger_count<=input$range_passenger[2] & 
...
#              Trip_type %in% input$trip_type & RateCodeID %in% input$rate_code)
#   })
#   colorpalette <- reactive({
#     colorNumeric('Spectral',y$Tip_ratio)
#   })
#   
...
#       clearShapes() %>% 
#       addCircles(radius = 50, weight = 1, color = "dodgerblue", fillColor = ~pal(Tip_ratio),
...
server <- function(input, output) {
  ...
  colorpalette <- reactive({
    colorNumeric('Spectral',data$Tip_ratio)
  })
  ...
  observe({
    ...
    leafletProxy("map", data=filteredData()) %>% 
      clearShapes() %>% 
      addCircles(radius = 200, weight = 1, color = "#888888", fillColor = ~pal(Tip_ratio),
                 fillOpacity = 0.7, popup = ~paste(Tip_amount)) %>% 
      clearControls() %>%
      addLegend(position = "bottomleft",pal = pal, values = ~Tip_ratio, opacity = 0.7, title = 'Tip (%)')
  })
